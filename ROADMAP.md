...
- **DB:** SQLite vía `modernc.org/sqlite` (pure Go, sin CGO). Ruta: `UserConfigDir()/computo-de-obra/computo.db`. Migraciones embebidas en `internal/infra/sqlite/migrations/` (0001 catálogos, 0002 series/versiones, 0003 snapshot, **0004 seed rubros** (`RUBROS.txt`), **0005 seed ítems** (`docs/ITEMS_.txt`), **0006 seed mano de obra** (`docs/MANODEOBRA.txt`), **0007 seed materiales** (`docs/MATERIALES.txt`), **0008 seed composición ítems** (`docs/ITEMS_ASOCIADO_MATERIALES .txt`)); tabla `_schema_version` para aplicar solo pendientes.
...
- **Implementado:** `ComputoList()`, `ComputoCreate(descripcion, superficieMilli, fechaInicio)` en `internal/app`; repo en `internal/infra/sqlite/repositories/computo_repo.go`.  
  **Extensión:** materiales extra por **versión + ítem** (`computo_item_material_extra`) con cantidad fija (`cantidad_milli`, no dosaje), sin tocar catálogo global de composición; incluye alta/listado/eliminación y restricción de no duplicar componente por `(version_id, item_id, componente_id)`.
